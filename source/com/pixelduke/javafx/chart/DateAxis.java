...
public class DateAxis extends ValueAxis<Long> {
	 ...
	 * increasingly bigger)
	 */
	private static final double[] TICK_UNIT_DEFAULTS = { 60000, // 1 minute
			120000, // 2 minute
			180000, // 3 minute
			240000, // 4 minute
			300000, // 5 minute
	...
	private static final String[] TICK_UNIT_FORMATTER_DEFAULTS = { "HH:mm", // 1 minute
			"HH:mm", // 2 minute
			"HH:mm", // 3 minute
			"HH:mm", // 4 minute
			"HH:mm", // 5 minute
	...
	/**
	 * When true zero is always included in the visible range. This only has effect
	 * if auto-ranging is on.
	 */
	...
	/**
	 * Create a non-auto-ranging DateAxis with the given upper bound, lower bound
	 * and tick unit
	 *
	...
	protected Object getRange() {
		...
		return new double[] { newMin, newMax, newTickUnit, getScale(), newIndex };
	}
	...
	private double[] recalculateTicks() {
		...
		int rangeIndex = 10;
		// loop till we find a set of ticks that fit length and result in a total of
		// less than 20 tick marks
		while (reqLength > length || count > 20) {
			// find a user friendly match from our default tick units to match calculated
			// tick unit
			...
			// calculate the required length to display the chosen tick marks for real, this
			// will handle if there are
	...
	protected void setRange(Object range, boolean animate) {
		...
		ReadOnlyDoubleWrapper scalePropertyImplValue = (ReadOnlyDoubleWrapper) ReflectionUtils
				.forceMethodCall(ValueAxis.class, "scalePropertyImpl", this);
...
//        // pad min and max by 2%, checking if the range is zero
	...
	/**
	 * Default number formatter for DateAxis, this stays in sync with auto-ranging
	 * and formats values appropriately. You can wrap this formatter to add prefixes
	 * or suffixes;
	 * 
	 * @since JavaFX 2.0
	 */
	public static class DefaultFormatter extends StringConverter<Long> {
		...
		/**
		 * Construct a DefaultFormatter for the given DateAxis with a prefix and/or
		 * suffix.
		 *
		 * @param axis   The axis to format tick marks for
		 * @param prefix The prefix to append to the start of formatted number, can be
		 *               null if not needed
		 * @param suffix The suffix to append to the end of formatted number, can be
		 *               null if not needed
		 */
		...
		private static TimeStringConverter getFormatter(int rangeIndex) {
			if (rangeIndex < 0) {
				return new TimeStringConverter("HH:mm");
		 ...
		 * string is defined by this converter.
		 * 
		...
		private String toString(Long object, int rangeIndex) {
			formatter = getFormatter(rangeIndex);
			return toString(object, formatter);
		}

		private String toString(Long time, TimeStringConverter formatter) {
			tempDate.setTime(time);
			String dateAsString = formatter.toString(tempDate);
			if (prefix != null && suffix != null) {
				return prefix + dateAsString + suffix;
			} else if (prefix != null) {
				return prefix + dateAsString;
			} else if (suffix != null) {
				return dateAsString + suffix;
			} else {
				return dateAsString;
			}
		}
		 ...
		 * Format of the string and type of the resulting object is defined by this
		 * converter.
		 * 
